...
function RADIO_saveFeedSource($id, $data, &$error)
{
    ...
    if ((int)$id > 0) {
        ...
        DB_query("UPDATE {$_TABLES['radio_sources']} SET title='".DB_escapeString(substr($title,0,255))."',source_url='".DB_escapeString($url)."',provider='".DB_escapeString(substr($provider,0,255))."',enabled=".$enabled.",group_id=".$groupId.",perm_owner=".$po.",perm_group=".$pg.",perm_members=".$pm.",perm_anon=".$pa.",modified='".DB_escapeString($now)."' WHERE source_id=".(int)$id);
    ...
    DB_query("INSERT INTO {$_TABLES['radio_sources']} (title,source_type,source_url,provider,enabled,owner_id,group_id,perm_owner,perm_group,perm_members,perm_anon,created,modified) VALUES ('".DB_escapeString(substr($title,0,255))."','rss','".DB_escapeString($url)."','".DB_escapeString(substr($provider,0,255))."',".$enabled.",".$owner.",".$groupId.",".$po.",".$pg.",".$pm.",".$pa.",'".DB_escapeString($now)."','".DB_escapeString($now)."')");
